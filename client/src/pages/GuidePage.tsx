...
import { Clock, Calendar, ArrowRight, BookOpen, ChevronRight, TrendingUp } from "lucide-react";
import { Footer } from "@/components/Footer";
import IndustryGuidePage from "./IndustryGuidePage";

export default function GuidePage() {
  const { slug } = useParams<{ slug: string }>();
  const guide = getPillarBySlug(slug || "");
  const industryGuide = getGuideBySlug(slug || "");

  if (!guide && industryGuide) {
    return <IndustryGuidePage />;
  }
